...
async function scrapeToAiPipeline(
  ...
  const { aiUrlSource } = autoPostData;
  const { gptModel, body, jsonResponse, mdTransformPrompt } = aiSettings;
  ...
  const author = draftFrontmatter?.author || null;
  ...
  if (frontmatter?.linkAiDraft && !autoPost) {
  ...
  const promptText = Object.values(
    await prompts(
      body,
      frontmatter.author || author || authorsData,
      content,
      mdTransformPrompt,
      jsonResponse,
      i18n,
