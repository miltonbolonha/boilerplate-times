...
const userRobots = async (postsDatas, pagesDatas, siteUrl) => {
  if (!postsDatas)
    return console.log("❌ [Posts Robots]: ERROR."), process.exit(1);
  if (!pagesDatas)
    return console.log("❌ [Pages Robots]: ERROR."), process.exit(1);

  const postsSlugs = postsDatas.map((post) => `Allow: ${"/" + post.slug}`);
  const pagesSlugs = pagesDatas.allPages.map(
    (page) => `Allow: ${"/" + page.slug}`
  );
  const allowSlugs = pagesSlugs.concat(postsSlugs);
  async function allowsStrings(slugs) {
    return slugs
      .map((allow) => {
  ...
  return `# robotstxt.org/ special tks to @github/mitchellkrogza

User-agent: *
${await allowsStrings(allowSlugs)}
Allow: /atom-feed.xml
Allow: /rss-feed.xml
Disallow: /draft/*
Disallow: /admin/*
Disallow: /wp-admin/*
Disallow: /wp-content/*
Disallow: /wp-json/tdw/save_css
Disallow: /wp-login.php
Disallow: /dental-implant-tech-disrupts-the-industry-jwgdn
Disallow: /wp-content/uploads/2016/11/1200x628_dentures_techleft_rightinstall.jpg
Disallow: /wp-content/uploads/2015/04/800x572_cruises_vacationersonsand.jpg
Disallow: /index.php
Disallow: /about.php
Disallow: /moon.php
Disallow: /admin.php
Disallow: /cgi-bin/xmrlpc.php
Disallow: /simple.php
Disallow: /xmrlpc.php
Disallow: /comments/feed
Disallow: /wp-admin/images/xmrlpc.php
